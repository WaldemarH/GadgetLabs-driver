...
//Get file path.
	...
	//Get file.
	HMODULE			hMod = GetModuleHandle( szdllname );
	if ( hMod == NULL )
		return ERRSREG_MODULE_NOT_FOUND;

	//Get path.
	GetModuleFileName( hMod, szModuleName, MAX_PATH_LEN );
	if ( szModuleName[0] == 0 )
		return ERRSREG_MODPATH_NOT_FOUND;

	CharLower( (LPTSTR)szModuleName );

//Get CLSID in text form.
...
//Is CLSID already present?
	sprintf_s( szregpath, sizeof( szregpath ), "%s\\%s", SZREGSTR_CLSID, szclsid );

	rc = findRegPath( HKEY_CLASSES_ROOT, szregpath );
	if (rc)
	{
	//CLSID is present... is InProcServer32 also present.
		sprintf_s( szregpath, sizeof( szregpath ), "%s\\%s\\%s", SZREGSTR_CLSID, szclsid, SZREGSTR_INPROCSERVER32 );
		...
		{
		//Delete old regentry
			sprintf_s( szregpath, sizeof( szregpath ), "%s", SZREGSTR_CLSID );
...
//HKEY_LOCAL_MACHINE\SOFTWARE\ASIO
	sprintf_s( szregpath, sizeof( szregpath ), "%s\\%s", SZREGSTR_SOFTWARE, SZREGSTR_ASIO );
	rc = findRegPath( HKEY_LOCAL_MACHINE, szregpath );

	if (rc)
	{
		sprintf_s( szregpath, sizeof( szregpath ), "%s\\%s\\%s", SZREGSTR_SOFTWARE, SZREGSTR_ASIO, szregname );
		rc = findRegPath( HKEY_LOCAL_MACHINE, szregpath );
		if (rc)
		{
			sprintf_s( szregpath, sizeof( szregpath ), "%s\\%s", SZREGSTR_SOFTWARE, SZREGSTR_ASIO );
	...
	{
	//HKEY_LOCAL_MACHINE\SOFTWARE\ASIO
		sprintf_s( szregpath, sizeof( szregpath ), "%s", SZREGSTR_SOFTWARE );
...

{
	...
	sprintf_s( newregpath, sizeof( newregpath ), "%s\\%s", szregpath, sznewpath );
...

{
	...
	if (cr == ERROR_SUCCESS)
	{
		size_t		size = strlen(subkeybuf)+1;

		newkey = new char[size];

		strcpy_s( newkey, size, subkeybuf);
...

{
	...
	if ((cr = RegOpenKey(mainkey,szregpath,&hkey)) == ERROR_SUCCESS)
	{
		size_t		size = strlen(szdelpath)+1;

		keyname = new char[size];
		...
		strcpy_s(keyname, size, szdelpath);
